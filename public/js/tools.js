...
window.DevTools = {
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-6 space-y-4">
          <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl">
            <div class="flex items-center justify-between mb-3">
              <label class="text-xs font-bold uppercase tracking-wider text-slate-300">${t('calcVerifierLabel')}</label>
            ...
            <textarea id="calc-verifier-input" dir="ltr" rows="3" oninput="window.DevTools.recalculatePkce()" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs font-mono text-amber-300 focus:outline-none focus:border-indigo-500">${window.PKCEEngine.generateCodeVerifier(64)}</textarea>
          ...
          <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl">
            <h3 class="text-xs font-bold uppercase tracking-wider text-slate-300 mb-3">${t('calcChallengeLabel')}</h3>
            <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2">
              <div class="text-[11px] text-slate-400" dir="ltr">Formula: <code class="text-emerald-400">BASE64URL(SHA256(ASCII(code_verifier)))</code></div>
              <div id="calc-challenge-output" dir="ltr" class="p-3 bg-slate-900 rounded-lg text-xs font-mono text-emerald-300 break-all select-all border border-slate-800">
                <!-- Calculated output -->
              </div>
            </div>
          </div>
        ...
        <div class="lg:col-span-6 space-y-4">
          <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            <h3 class="text-xs font-bold uppercase tracking-wider text-slate-300">${t('calcStepsTitle')}</h3>
            
            <div class="space-y-3 text-xs font-mono">
              <div class="p-3 bg-slate-950 rounded-xl border border-slate-800">
                <div class="text-slate-400 text-[10px] font-sans font-semibold uppercase mb-1">${t('calcStep1')}</div>
                <div id="calc-step-ascii" class="text-indigo-300 break-all text-[11px]" dir="ltr"></div>
              </div>

              <div class="p-3 bg-slate-950 rounded-xl border border-slate-800">
                <div class="text-slate-400 text-[10px] font-sans font-semibold uppercase mb-1">${t('calcStep2')}</div>
                <div id="calc-step-sha256" class="text-purple-300 break-all text-[11px]" dir="ltr"></div>
              </div>

              <div class="p-3 bg-slate-950 rounded-xl border border-slate-800">
                <div class="text-slate-400 text-[10px] font-sans font-semibold uppercase mb-1">${t('calcStep3')}</div>
                <div id="calc-step-base64url" class="text-emerald-300 break-all text-[11px]" dir="ltr"></div>
              </div>
            </div>
          </div>
        </div>
    ...
    if (validEl) {
      if (isValidLen && isValidChars) {
        validEl.innerHTML = `
          ...
          <span class="text-slate-400 font-mono" dir="ltr">${len} / 128 chars</span>
        `;
      } else {
        validEl.innerHTML = `
          ...
          <span class="text-rose-400 font-mono" dir="ltr">${len} chars</span>
        `;
      }
    }
  ...
  renderJwtDebugger() {
    ...
    return `
      <div class="space-y-6">
        <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl">
          <label class="text-xs font-bold uppercase tracking-wider text-slate-300 block mb-2">${t('jwtPasteLabel')}</label>
          <textarea id="jwt-input-field" dir="ltr" rows="4" oninput="window.DevTools.decodeJwtInput()" placeholder="eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..." class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs font-mono text-cyan-300 focus:outline-none focus:border-indigo-500"></textarea>
        </div>
        ...
        <div id="jwt-decoded-container" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div class="lg:col-span-4 bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl">
            <h4 class="text-xs font-bold uppercase tracking-wider text-rose-400 mb-3">${t('jwtHeader')}</h4>
            <pre id="jwt-header-display" class="bg-slate-950 p-3 rounded-xl text-xs font-mono text-rose-300 overflow-x-auto" dir="ltr">Paste a token above to inspect...</pre>
          </div>

          <div class="lg:col-span-8 bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl">
            <h4 class="text-xs font-bold uppercase tracking-wider text-purple-400 mb-3">${t('jwtPayload')}</h4>
            <pre id="jwt-payload-display" class="bg-slate-950 p-3 rounded-xl text-xs font-mono text-purple-300 overflow-x-auto" dir="ltr">Paste a token above to inspect...</pre>
          </div>
        </div>
      </div>
    `;
  ...
  renderDiscoveryInspector() {
    ...
    return `
      <div class="space-y-6">
        <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl">
          <label class="text-xs font-bold uppercase tracking-wider text-slate-300 block mb-2">${t('discoveryInputLabel')}</label>
          <div class="flex items-center gap-2">
            <input type="text" id="discovery-issuer-input" dir="ltr" value="http://localhost:3000/mock-idp" placeholder="https://accounts.google.com or https://your-tenant.auth0.com" class="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-100 font-mono">
        ...
        <div id="discovery-results" class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl">
          <div class="text-xs text-slate-400 italic">${window.i18n.currentLang === 'ar' ? 'اضغط على زر الفحص لاختبار بيانات المزود.' : 'Click "Inspect Provider Metadata" to test the issuer.'}</div>
        </div>
      </div>
    `;
  },
      ...
      results.innerHTML = `
        <div class="space-y-4">
          <div class="flex items-center justify-between pb-3 border-b border-slate-800">
            ...
            <span class="px-2.5 py-1 rounded-full text-xs font-semibold ${data.supportsS256 ? 'bg-emerald-950 text-emerald-300 border border-emerald-800' : 'bg-rose-950 text-rose-300 border border-rose-800'}" dir="ltr">
              ${data.supportsS256 ? '✓ S256 PKCE Supported' : '✗ S256 PKCE Missing'}
            </span>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs font-mono">
            <div class="p-3 bg-slate-950 rounded-xl border border-slate-800" dir="ltr"><span class="text-slate-400">Authorization Endpoint:</span> <span class="text-indigo-300">${data.authorization_endpoint || 'N/A'}</span></div>
            <div class="p-3 bg-slate-950 rounded-xl border border-slate-800" dir="ltr"><span class="text-slate-400">Token Endpoint:</span> <span class="text-emerald-300">${data.token_endpoint || 'N/A'}</span></div>
            <div class="p-3 bg-slate-950 rounded-xl border border-slate-800" dir="ltr"><span class="text-slate-400">JWKS URI:</span> <span class="text-amber-300">${data.jwks_uri || 'N/A'}</span></div>
            <div class="p-3 bg-slate-950 rounded-xl border border-slate-800" dir="ltr"><span class="text-slate-400">UserInfo Endpoint:</span> <span class="text-cyan-300">${data.userinfo_endpoint || 'N/A'}</span></div>
          </div>

          <div class="mt-3">
            <div class="text-xs font-semibold text-slate-400 mb-1">Raw JSON Metadata</div>
            <pre class="bg-slate-950 p-3 rounded-xl text-xs font-mono text-slate-300 max-h-60 overflow-y-auto custom-scrollbar" dir="ltr"><code>${JSON.stringify(data.raw, null, 2)}</code></pre>
          </div>
        </div>
      `;
    } catch (err) {
  ...
  renderSecurityMatrix() {
    ...
    return `
      <div class="space-y-6">
        ...
        <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl">
          <h3 class="text-sm font-bold uppercase tracking-wider text-slate-300 mb-4">${t('storageMatrixTitle')}</h3>
            ...
            <table class="w-full text-start text-xs">
              ...
              <tbody class="divide-y divide-slate-800/60 font-mono text-slate-300">
                <tr>
                  ...
                  <td class="p-3 text-amber-300" dir="ltr">In-Memory / SessionStorage</td>
                  <td class="p-3 text-rose-400" dir="ltr">High (if in localStorage)</td>
                  <td class="p-3 text-emerald-400" dir="ltr">None (Bearer header)</td>
                ...
                <tr>
                  ...
                  <td class="p-3 text-emerald-300" dir="ltr">HttpOnly Encrypted Cookie</td>
                  <td class="p-3 text-emerald-400" dir="ltr">Immune (JS cannot read cookie)</td>
                  <td class="p-3 text-amber-400" dir="ltr">Mitigated via SameSite=Lax + Anti-CSRF</td>
                  <td class="p-3 font-sans text-indigo-300">Confidential Client with PKCE S256</td>
                </tr>
                <tr>
                  <td class="p-3 font-sans font-semibold text-white">Mobile Flutter (iOS & Android)</td>
                  <td class="p-3 text-emerald-300" dir="ltr">FlutterSecureStorage (Keychain / Keystore)</td>
                  <td class="p-3 text-emerald-400" dir="ltr">None (Native binary)</td>
                  <td class="p-3 text-emerald-400" dir="ltr">None</td>
                  <td class="p-3 font-sans text-indigo-300">flutter_appauth + ASWebAuth / Custom Tabs</td>
                </tr>
                <tr>
                  <td class="p-3 font-sans font-semibold text-white">Mobile iOS (Swift / SwiftUI)</td>
                  <td class="p-3 text-emerald-300" dir="ltr">iOS Keychain (SecItem)</td>
                  <td class="p-3 text-emerald-400" dir="ltr">None (No browser DOM)</td>
                  <td class="p-3 text-emerald-400" dir="ltr">None</td>
                ...
                <tr>
                  ...
                  <td class="p-3 text-emerald-300" dir="ltr">EncryptedSharedPreferences (Keystore)</td>
                  <td class="p-3 text-emerald-400" dir="ltr">None</td>
                  <td class="p-3 text-emerald-400" dir="ltr">None</td>
